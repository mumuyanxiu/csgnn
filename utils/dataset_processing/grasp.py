...
class GraspRectangles:
    ...
    def __getattr__(self, attr):
        """
        Test if GraspRectangle has the desired attr as a function/property and call it.
        """
        # 首先检查是否是 GraspRectangles 自己的属性（如 center property）
        if hasattr(type(self), attr):
            # 获取类属性
            class_attr = getattr(type(self), attr)
            # 如果是 property，直接调用
            if isinstance(class_attr, property):
                return class_attr.fget(self)
        
        # 检查 GraspRectangle 是否有这个属性
        if hasattr(GraspRectangle, attr):
            gr_attr = getattr(GraspRectangle, attr)
            # 如果是方法，返回 lambda 映射到所有 grs
            if callable(gr_attr):
                return lambda *args, **kwargs: list(map(lambda gr: getattr(gr, attr)(*args, **kwargs), self.grs))
            # 如果是 property，获取所有 gr 的该属性值
            elif isinstance(gr_attr, property):
                return [getattr(gr, attr) for gr in self.grs]
        
        raise AttributeError("Couldn't find function %s in GraspRectangles or GraspRectangle" % attr)
