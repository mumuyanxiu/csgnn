...
class DepthImage(Image):
    ...
    def inpaint(self, missing_value=0):
        ...
        """
        # 简化版本：不使用 inpaint，直接用插值填充缺失值
        # OpenCV 4.12.0 在某些环境下 inpaint 有兼容性问题
        
        # 方法：用最近邻填充 0 值
        mask = (self.img == missing_value)
        
        if mask.any():
            # 使用中值填充缺失值
            from scipy import ndimage
            # 对于缺失的点，用周围的平均值填充
            self.img = ndimage.grey_opening(self.img, size=(3, 3))
            ...
            if mask.all():
                ...
                # 用非缺失值的中值填充
                median_val = np.median(self.img[~mask])
                self.img[mask] = median_val
